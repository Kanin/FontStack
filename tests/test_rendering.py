"""Integration tests for draw_text_smart and render_text (requires real fonts)."""
...
# region draw_text_smart


class TestDrawTextSmart:
    ...
    def test_returns_nonzero_dimensions(self, font_manager: FontManager) -> None:
        img = self._canvas()
        w, h = font_manager.draw_text_smart(img, "Hello", position=(10, 10), size=32)
        assert w > 0
        assert h > 0

    def test_empty_string_returns_zero(self, font_manager: FontManager) -> None:
        img = self._canvas()
        result = font_manager.draw_text_smart(img, "", position=(10, 10), size=32)
        assert result == (0, 0)

    def test_modifies_image_in_place(self, font_manager: FontManager) -> None:
        img = self._canvas()
        original_id = id(img)
        font_manager.draw_text_smart(img, "Hello", position=(10, 10), size=32)
    ...
    def test_wrap_mode_multiline_taller_than_singleline(
        ...
        _, h_wrapped = font_manager.draw_text_smart(
            img,
        ...
        _, h_single = font_manager.draw_text_smart(
            img2,
            "Hello",
            position=(0, 0),
            size=32,
    ...
    def test_scale_mode_respects_max_width(self, font_manager: FontManager) -> None:
        ...
        max_w = 200
        w, _ = font_manager.draw_text_smart(
            img,
            "Hello world",
            position=(0, 0),
            size=48,
    ...
    def test_invalid_mode_raises(self, font_manager: FontManager) -> None:
        ...
        with pytest.raises(ValueError, match="mode must be"):
            font_manager.draw_text_smart(img, "Hello", position=(0, 0), mode="invalid")  # type: ignore[arg-type]

    def test_invalid_align_raises(self, font_manager: FontManager) -> None:
        img = self._canvas()
        with pytest.raises(ValueError, match="align must be"):
            font_manager.draw_text_smart(img, "Hello", position=(0, 0), align="middle")  # type: ignore[arg-type]

    def test_arabic_text_renders(self, font_manager: FontManager) -> None:
        img = self._canvas()
        w, h = font_manager.draw_text_smart(img, "مرحبا", position=(10, 10), size=32)
        assert w > 0
        assert h > 0

    def test_mixed_script_renders(self, font_manager: FontManager) -> None:
        img = self._canvas((1000, 200))
        w, h = font_manager.draw_text_smart(
            img, "Hello مرحبا", position=(10, 10), size=32
        )
        assert w > 0
        assert h > 0
    ...
    def test_variable_font_weight(self, font_manager: FontManager) -> None:
        ...
        w_thin, h_thin = font_manager.draw_text_smart(
            img_thin, "Hello", position=(0, 0), size=40, weight=100
        )
        w_bold, h_bold = font_manager.draw_text_smart(
...
class TestRenderText:
    def test_returns_image(self, default_stack: list[FontConfig]) -> None:
        img = render_text("Hello", font_stack=default_stack, size=32)
    ...
    def test_padding_increases_dimensions(
        ...
        no_pad = render_text("Hello", font_stack=default_stack, size=32, padding=0)
        padded = render_text("Hello", font_stack=default_stack, size=32, padding=20)
    ...
    def test_no_stack_no_manager_raises(self) -> None:
        with pytest.raises(ValueError, match="Provide at least one FontConfig"):
            render_text("Hello", font_stack=[])

    def test_reuse_manager_produces_same_output(
        self, font_manager: FontManager, default_stack: list[FontConfig]
    ) -> None:
        img_direct = render_text("Hello", font_stack=default_stack, size=32)
        img_mgr = render_text("Hello", font_stack=[], manager=font_manager, size=32)
    ...
    def test_render_text_no_max_width_reasonable_canvas(
        self, default_stack: list[FontConfig]
    ) -> None:
        """Unconstrained render_text must not balloon the canvas."""
        text = "Hello world"
        size = 40
        img = render_text(text, font_stack=default_stack, size=size)
...
class TestPrepareBidi:
    ...
    def test_mixed_arabic_latin_renders_without_error(
        ...
        w, h = font_manager.draw_text_smart(
            img, "Hello مرحبا world", position=(10, 10), size=32
        )
        assert w > 0 and h > 0
...
class TestFitMode:
    ...
    def test_fit_mode_respects_max_height(self, font_manager: FontManager) -> None:
        img = self._canvas((800, 400))
        max_h = 80
        _, h = font_manager.draw_text_smart(
            img,
    ...
    def test_fit_mode_via_render_text(self, default_stack: list[FontConfig]) -> None:
        img = render_text(
    ...
    def test_fit_mode_shrinks_font_for_tall_content(
        ...
        _, h_large = font_manager.draw_text_smart(
            img_large,
            text,
            position=(0, 0),
            size=40,
        ...
        _, h_small = font_manager.draw_text_smart(
            img_small,
            text,
            position=(0, 0),
            size=40,
